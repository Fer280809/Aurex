...
const handler = async (m, { conn }) => {
    ...
    if (alreadyHas) {
        // Dar monedas como compensación por duplicado
        const compensation = Math.floor(tempData.character.value * 0.3);
        global.db.data.users[userId].coin += compensation;
        
        delete global.tempCharacters[quotedId];
        return m.reply(`⚠️ *Ya tienes este personaje.*\nRecibes *¥${compensation}* de compensación.`);
    }
